...
import { ProjectServiceType } from '../type.type';
...
export class ListingComponent implements OnInit {
    ...
    public data: ProjectServiceType[];
    displayedColumns: string[] = ['no', 'name', 'color', 'action'];
    dataSource: MatTableDataSource<ProjectServiceType>;
    constructor(
        private readonly _typeService: ServiceTypeService,
            ...
            if (result) {
                this._typeService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: ProjectServiceType) => v.id !== id);
    ...
    listing(): void {
        ...
        this.loading = true;
        this._typeService.read().subscribe({
            next: (res: ProjectServiceType[]) => {
                this.loading = false;
                this.data = res;
                this.dataSource = new MatTableDataSource<ProjectServiceType>(this.data);

...
import { TechFramework } from '../framework.type';
...
export class ListingComponent implements OnInit {
    ...
    public data: TechFramework[];
    displayedColumns: string[] = ['no', 'icon', 'name','language', 'updated_at', 'action'];
    dataSource: MatTableDataSource<TechFramework>;
    constructor(
    ...
    delete(id: number = 0): void {
        ...
        dialogRef.afterClosed().subscribe((result: boolean) => {
            if (result) {
                this._frameworkService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: TechFramework) => v.id !== id);
    ...
    listing(): void {
        ...
        this.loading = true;
        this._frameworkService.read().subscribe({
            next: (res:any) => {
                this.loading = false;
                // console.log(res); 
                ...
                this.dataSource = new MatTableDataSource<TechFramework>(this.data);

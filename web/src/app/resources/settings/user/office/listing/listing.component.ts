...
import { UserOffice } from '../office.type';
...
export class ListingComponent implements OnInit {
    ...
    public data: UserOffice[];
    displayedColumns: string[] = ['no', 'kh_name', 'en_name', 'action'];
    dataSource: MatTableDataSource<UserOffice>;
    constructor(
            ...
            if (result) {
                this._officeService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: UserOffice) => v.id !== id);
    ...
    listing(): void {
        ...
        this.loading = true;
        this._officeService.read().subscribe({
            next: (res: UserOffice[]) => {
                this.loading = false;
                this.data = res;
                this.dataSource = new MatTableDataSource<UserOffice>(this.data);

...
import { UserDepartment } from '../organization.type';
...
export class ListingComponent implements OnInit {
    ...
    public data: UserDepartment[];
    displayedColumns: string[] = ['no', 'kh_name', 'en_name', 'action'];
    dataSource: MatTableDataSource<UserDepartment>;
    constructor(
            ...
            if (result) {
                this._orgService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: UserDepartment) => v.id !== id);
    ...
    listing(): void {
        ...
        this.loading = true;
        this._orgService.read().subscribe({
            next: (res: UserDepartment[]) => {
                this.loading = false;
                this.data = res;
                this.dataSource = new MatTableDataSource<UserDepartment>(this.data);

...
import { UserPosition } from '../position.type';
...
export class ListingComponent implements OnInit {
    ...
    public data: UserPosition[];
    displayedColumns: string[] = ['no', 'kh_name', 'en_name', 'action'];
    dataSource: MatTableDataSource<UserPosition>;
    constructor(
    ...
    delete(id: number = 0): void {
        ...
        dialogRef.afterClosed().subscribe((result: boolean) => {
            if (result) {
                this._positionService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: UserPosition) => v.id !== id);
    ...
    listing(): void {
        ...
        this.loading = true;
        this._positionService.read().subscribe({
            next: (res: UserPosition[]) => {
                this.loading = false;
                this.data = res;
                this.dataSource = new MatTableDataSource<UserPosition>(this.data);

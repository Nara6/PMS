...
export class ListingComponent implements OnInit {
    ...
    dataSource: MatTableDataSource<VMEnv>;
    constructor(
    ...
    delete(id: number = 0): void {
        ...
        dialogRef.afterClosed().subscribe((result: boolean) => {
            if (result) {
                this._envService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: VMEnv) => v.id !== id);
    ...
    listing(): void {
        ...
        this.loading = true;
        this._envService.read().subscribe({
            next: (res: VMEnv[]) => {
                this.loading = false;
                this.data = res;
                this.dataSource = new MatTableDataSource<VMEnv>(this.data);

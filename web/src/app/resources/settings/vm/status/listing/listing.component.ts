...
import { VMStatus } from '../status.type';
...
export class ListingComponent implements OnInit {
    ...
    public data: VMStatus[];
    displayedColumns: string[] = ['no', 'name', 'color', 'action'];
    dataSource: MatTableDataSource<VMStatus>;
    constructor(
            ...
            if (result) {
                this._statusService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: VMStatus) => v.id !== id);
    ...
    listing(): void {
        ...
        this._statusService.read().subscribe({
            next: (res: VMStatus[]) => {
                this.loading = false;
                this.data = res;
                this.dataSource = new MatTableDataSource<VMStatus>(this.data);

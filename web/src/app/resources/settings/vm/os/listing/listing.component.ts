...
export class ListingComponent implements OnInit {
    ...
    public data: VMOs[];
    displayedColumns: string[] = ['no', 'icon', 'name', 'action'];
    dataSource: MatTableDataSource<VMOs>;
    constructor(
    ...
    delete(id: number = 0): void {
        ...
        dialogRef.afterClosed().subscribe((result: boolean) => {
            if (result) {
                this._osService.delete(id).subscribe({
                    next: (response: { statusCode: number, message: string }) => {
                        this.data = this.data?.filter((v: VMOs) => v.id !== id);
    ...
    listing(): void {
        ...
        this.loading = true;
        this._osService.read().subscribe({
            next: (res: VMOs[]) => {
                this.loading = false;
                this.data = res;
                this.dataSource = new MatTableDataSource<VMOs>(this.data);

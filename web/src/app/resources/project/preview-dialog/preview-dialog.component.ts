...
export class PreviewDialogComponent implements OnInit {
  ...
  ngBuilderForm(): void {
    this.data_preview = this.formBuilder.group({
      ...
      status_id: [this.data.ProjectsStatus.id, Validators.required],
      type_id: [this.data.ProjectsType.id, Validators.required],
      start_date: [new Date(this.data.start_date), Validators.required],
      due_date: [new Date(this.data.due_date), Validators.required],
      kh_name: [this.data.kh_name],
      abbre: [this.data.abbre],
      icon: [this.data.icon]
    });
  }
  ngOnChanges() {
  ...
  submit(): void {
    ...
    this.loading = true;
    this.projectService.update(this.data.id, this.data_preview.value).subscribe({
      next: (res) => {
        this.loading = false;
